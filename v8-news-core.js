...
async function fetchCategory(url){
 const direct=async()=>{const c=new AbortController(),t=setTimeout(()=>c.abort(),6000);try{const r=await fetch(url,{signal:c.signal,redirect:'follow',headers:{'User-Agent':UA,Accept:'application/rss+xml,application/atom+xml,application/xml,text/xml,text/plain'}});if(!r.ok)throw Error('HTTP '+r.status);return{r,transport:'direct'}}finally{clearTimeout(t)}};
 try{return await direct()}catch(first){const c=new AbortController(),t=setTimeout(()=>c.abort(),6000);try{const r=await fetch('https://r.jina.ai/'+url,{signal:c.signal,redirect:'follow',headers:{'User-Agent':UA,Accept:'text/plain'}});if(r.ok)return{r,transport:'proxy'};throw Error('HTTP '+r.status)}finally{clearTimeout(t)}}
}
const buckets=Object.fromEntries(GROUPS.map(k=>[k,[]]));
const states=await Promise.all(categoryFeeds.map(async f=>{const q=encodeURIComponent(f[2]+' when:7d'),url='https://news.google.com/rss/search?q='+q+'&hl=en-US&gl=US&ceid=US:en';try{const got=await fetchCategory(url),text=await got.r.text(),items=parse(text,[f[1],f[0],url]).map(x=>({...x,score:sentiment(x),cat:f[0],type:f[0]}));return{name:f[1],type:f[0],url,status:'ok',count:items.length,transport:got.transport,items}}catch(e){return{name:f[1],type:f[0],url,status:'error',count:0,error:String(e.message||e),items:[]}}}));
