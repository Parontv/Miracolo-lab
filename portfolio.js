/* Miracolo Lab — Manual Portfolio / V3.8 */
(function(){
 ...
 function render(){const items=load(),host=document.getElementById('portfolioRows');if(!host)return;const total=items.reduce((a,x)=>a+(Number(x.value)||0),0);document.getElementById('portfolioTotal').textContent=money(total);const stat=document.getElementById('portfolioStat');if(stat)stat.textContent=money(total);host.innerHTML=items.length?items.map((x,i)=>`<div class="holding"><div><b>${esc(x.symbol)}</b><span>${esc(x.name||x.type||'Asset')}</span></div><div><b>${money(x.value)}</b></div><button class="holding-del" data-i="${i}" title="Rimuovi">×</button></div>`).join(''):'<div class="portfolio-empty">Nessun asset inserito. Aggiungi manualmente BTC, ETF o altri strumenti.</div>';host.querySelectorAll('.holding-del').forEach(b=>b.onclick=()=>{const a=load();a.splice(Number(b.dataset.i),1);save(a);render()})}
