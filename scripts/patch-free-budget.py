...
# Reduce RSS batch pressure. Five rescue calls can still fit under the 50
...
async function market(env){
  ...
  const out={timestamp:new Date().toISOString(),build:BUILD,marketUniverseVersion:'DATA-EXPANSION-V1',batch:{start:startAt,size:MARKET_BATCH,next},indices,crypto,institutionalData:institutional,socialData:social,onchainData:onchain};
