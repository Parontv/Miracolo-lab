...
old_sort="const weak=GROUPS.filter(k=>(buckets[k]?.length||0)<200).sort((a,b)=>(buckets[b]?.length||0)-(buckets[a]?.length||0));"
new_sort="const weak=GROUPS.filter(k=>(buckets[k]?.length||0)<200).sort((a,b)=>(buckets[b]?.length||0)-(buckets[a]?.length||0)).reverse().slice(0,5);"
if old_sort in s:s=s.replace(old_sort,new_sort)
else:
    old_sort2="const weak=GROUPS.filter(k=>(buckets[k]?.length||0)<200).sort((a,b)=>(buckets[b]?.length||0)-(buckets[a]?.length||0)).slice(0,5);"
