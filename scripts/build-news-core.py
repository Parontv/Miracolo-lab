...
p = Path('worker.js')
s = p.read_text()
s = s.replace("const BUILD='ML-20260906-V4-NEWS-TRANSPORT';", "const BUILD='ML-20260908-V16-25-FEEDS-12-GDELT-DENSITY';")
...
async function gdeltFeed(cat,label,query){try{const u='https://api.gdeltproject.org/api/v2/doc/doc?query='+encodeURIComponent(query)+'&mode=artlist&format=json&timespan=24h&maxrecords=250&sort=datedesc';const r=await get(u);if(!r.ok)throw Error('HTTP '+r.status);const d=await r.json();const raw=Array.isArray(d?.articles)?d.articles:[];const items=raw.map(x=>({title:String(x.title||'').trim(),description:'',date:x.seendate||x.date||'',url:x.url||x.url_mobile||'',link:x.url||x.url_mobile||'',source:label,type:cat,cat,publisher:x.domain||'GDELT'})).filter(x=>x.title&&x.url).map(x=>({...x,score:sentiment(x)}));return{name:label,type:cat,url:u,status:'ok',count:items.length,transport:'gdelt',items}}catch(e){return{name:label,type:cat,status:'error',count:0,error:String(e.message||e),items:[]}}}
const primary=FEEDS.slice(0,25);
...
const states=[...feedStates,...gdeltStates];const all=states.flatMap(x=>x.items);const buckets=Object.fromEntries(GROUPS.map(k=>[k,[]]));for(const x of all)buckets[categoryOf(x)].push(x);
