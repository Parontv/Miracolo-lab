...
p=Path('worker.js')
s=p.read_text()
s=s.replace("const BUILD='ML-20260906-V4-NEWS-TRANSPORT';","const BUILD='ML-20260909-V22-CUMULATIVE-SENTIMENT';")
...
# V22: count every bullish/bearish keyword occurrence across title + description,
# rather than returning only -1/0/+1. Keep the score bounded for the frontend.
old="function sentiment(i){const t=(String(i.title||'')+' '+String(i.description||'')).toLowerCase();const count=re=>(t.match(re)||[]).length;const bullish=count(/bullish|rally|surge|breakout|beat|inflow|upgrade|record high|easing|rate cut|strong growth/g);const bearish=count(/bearish|crash|dump|selloff|plunge|outflow|downgrade|recession|liquidat|hawkish|rate hike|panic|default/g);return Math.max(-6,Math.min(6,bullish-bearish))}"
new="function sentiment(i){const t=(String(i.title||'')+' '+String(i.description||'')).toLowerCase();const count=re=>(t.match(re)||[]).length;const bullish=count(/bullish|rally|surge|breakout|beat|inflow|upgrade|record high|easing|rate cut|strong growth/g);const bearish=count(/bearish|crash|dump|selloff|plunge|outflow|downgrade|recession|liquidat|hawkish|rate hike|panic|default/g);return Math.max(-6,Math.min(6,bullish-bearish))}"
if old not in s: raise SystemExit('sentiment function not found in generated worker')
s=s.replace(old,new)
...
p.write_text(s)
print('Built V22 cumulative sentiment scoring')
