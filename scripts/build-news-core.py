...
p = Path('worker.js')
s = p.read_text()
s = s.replace("const BUILD='ML-20260906-V4-NEWS-TRANSPORT';", "const BUILD='ML-20260909-V20-PERSISTENT-BATCHED-NEWS';")
s = s.replace("t=setTimeout(()=>c.abort(),20000)", "t=setTimeout(()=>c.abort(),12000)")

# Preserve explicit feed categories before keyword inference.
cat_pattern = r"function categoryOf\(x\)\{.*?\}\nasync function scan"
cat_fn = r'''function categoryOf(x){const raw=`${x.cat||x.type||''} ${x.source||''} ${x.title||''} ${x.description||''}`.toLowerCase(),t=String(x.cat||x.type||'').toLowerCase();if(GROUPS.includes(t))return t;if(t==='crypto'||/crypto|bitcoin|ethereum|solana|xrp|coindesk/.test(raw))return'crypto';if(/inflation|cpi|pce|gdp|payroll|pmi|fred|bls|bea|eurostat|liquidity/.test(raw))return'macro';if(/treasury|bond|yield|bund|btp|gilt|credit spread|high yield|cds/.test(raw))return'rates';if(/fed|ecb|boe|boj|snb|rba|central bank/.test(raw))return'central';if(/gold|silver|oil|brent|wti|copper|opec|gas/.test(raw))return'commodities';if(/forex|eurusd|eur\/usd|dxy|usd[jy]|gbp|sterling|dollar|yen/.test(raw))return'fx';if(/vix|volatility|options|gamma|put.?call|open interest/.test(raw))return'volatility';if(/geopolitic|sanction|tariff|war|conflict|ukraine|russia|iran|israel/.test(raw))return'geopolitics';if(/reddit|social|sentiment|wallstreetbets/.test(raw))return'social';if(/earnings|company|corporate|sec filing|revenue|guidance|buyback|jpmorgan|goldman|banking/.test(raw))return'company';if(/finance|market|stock|equity|etf|nasdaq|s&p|dow|dax|nikkei|ftse|msci/.test(raw))return'finance';return'news'}
async function scan'''
s2,n=re.subn(cat_pattern,cat_fn,s,count=1,flags=re.S)
if n!=1: raise SystemExit('category replacement failed')
s=s2
...
const cur=await readKV(env,CURSOR_KEY)||{next:0},previous=await readKV(env,SNAPSHOT_KEY),startAt=Number(cur.next||0)%FEEDS.length,selected=[];
for(let i=0;i<BATCH&&i<FEEDS.length;i++)selected.push(FEEDS[(startAt+i)%FEEDS.length]);
...
const weak=GROUPS.filter(k=>(buckets[k]?.length||0)<200).sort((a,b)=>(buckets[a]?.length||0)-(buckets[b]?.length||0)).slice(0,5);
for(const cat of weak){const f=RESCUE.find(x=>x[0]===cat);if(!f)continue;const extra=await fetchFeed(f);if(extra.status==='ok')buckets[cat].push(...extra.items.map(x=>({...x,rescue:true})));}
...
const items=GROUPS.flatMap(k=>buckets[k]),pos=items.filter(x=>x.score>0).length,neg=items.filter(x=>x.score<0).length;
const categorySummary=Object.fromEntries(GROUPS.map(k=>[k,{count:buckets[k].length,complete:buckets[k].length>=200}]));
...
for(const cat of weak){const f=RESCUE.find(x=>x[0]===cat);if(f)sourceMap.set(f[1],{name:f[1],type:f[0],url:f[2],status:'rescue'})}
...
p.write_text(s)
print('Built V20 persistent batched News Core')
