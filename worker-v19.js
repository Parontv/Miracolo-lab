/* Miracolo Lab V19 — source expansion wrapper; preserves V18 worker API */
...
async function get(url){const c=new AbortController(),t=setTimeout(()=>c.abort(),7000);try{return await fetch(url,{signal:c.signal,redirect:'follow',headers:{'User-Agent':'MiracoloLab/19.0',Accept:'application/rss+xml,application/atom+xml,application/xml,text/xml'}})}finally{clearTimeout(t)}}
function parse(txt,f){const out=[],re=/<(?:item|entry)(?:\s[^>]*)?>[\s\S]*?<\/(?:item|entry)>/gi;let m;while((m=re.exec(txt))){const x=m[0],title=field(x,'title'),description=field(x,'description')||field(x,'summary'),date=field(x,'pubDate')||field(x,'published')||field(x,'updated'),url=link(x);if(title)out.push({title,description,date,url,link:url,source:f[0],type:f[1],cat:f[1],score:0})}return out}
async function extraScan(){const states=await Promise.all(EXTRA.map(async f=>{try{const r=await get(f[2]);if(!r.ok)throw Error('HTTP '+r.status);const items=parse(await r.text(),f);return{name:f[0],type:f[1],url:f[2],status:'ok',count:items.length,items}}catch(e){return{name:f[0],type:f[1],url:f[2],status:'error',count:0,error:String(e.message||e),items:[]}}}));return{states,items:states.flatMap(x=>x.items)}}
const json=x=>Response.json(x,{headers:{'Cache-Control':'no-store','Access-Control-Allow-Origin':'*'}});
const cacheKey=new Request('https://miracolo-lab.local/v19-extra');
async function putExtra(x){try{await caches.default.put(cacheKey,json(x))}catch{}}
async function getExtra(){try{const r=await caches.default.match(cacheKey);return r?r.json():{states:[],items:[]}}catch{return{states:[],items:[]}}}
export default{async scheduled(event,env,ctx){if(base.scheduled)ctx.waitUntil(base.scheduled(event,env,ctx));ctx.waitUntil(extraScan().then(putExtra))},async fetch(req,env,ctx){const u=new URL(req.url);if(u.pathname==='/api/full-scan'){const baseRes=await base.fetch(new Request(new URL('/api/full-scan',u),req),env,ctx);const b=await baseRes.json();const e=await extraScan();const items=[...(b.items||[]),...e.items];const unique=[...new Map(items.map(x=>[(x.title||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim(),x])).values()].slice(0,3000);const sources=[...(b.sources||[]),...e.states.map(({items,...x})=>x)];return json({...b,items:unique,top_signals:unique,sources,summary:{...(b.summary||{}),configured:sources.length,reachable:sources.filter(x=>x.status==='ok').length,failed:sources.filter(x=>x.status==='error').length,items:unique.length,total:unique.length,workingSources:sources.filter(x=>x.status==='ok').length}})}return base.fetch(req,env,ctx)}};
