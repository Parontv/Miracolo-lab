/* Miracolo Lab V1 — Dashboard intelligence + Strategy Lab context. */
(() => {
  'use strict';
  ...
  function regimeLabel(ai){
    ...
    const labels={risk_on:'Risk-on',risk_off:'Risk-off',neutral:'Neutrale',mixed:'Misto',rialzista:'Rialzista',moderatamente_rialzista:'Moderatamente rialzista',ribassista:'Ribassista',moderatamente_ribassista:'Moderatamente ribassista'};
  ...
  function strategyContext(){
    const s=window.ML_STRATEGY_LAB?.consensus;
    if(!s)return null;
    return {
      ...
      modules:Array.isArray(s.modules)?s.modules.map(x=>({module:x.module,action:x.action,confidence:x.confidence,ret:x.ret,benchmark:x.benchmark,drawdown:x.drawdown,winRate:x.winRate,risk:x.risk,reason:x.reason})):[]
    };
  }
  ...
  function strategyNarrative(){
    ...
    if(!s)return '';
    const vote=`BUY ${s.votes.BUY||0} · SELL ${s.votes.SELL||0} · WAIT ${s.votes.WAIT||0}`;
    let text=`Strategy Lab: ${s.action} con confidenza ${s.confidence}% e concordanza ${s.agreement}%. ${vote}.`;
    if(s.risk==='ELEVATO')text+=' Il rischio viene considerato elevato perché i segnali non sono sufficientemente allineati.';
    if(s.contradictions.length)text+=` Contraddizioni rilevate: ${s.contradictions.join(', ')}.`;
    return text;
  }
  ...
  function renderMarketText(ai){
    ...
    const strategy=strategyNarrative();
    return `<div class="v1-ai-headline">${esc(headline)}</div>${label?`<div class="v1-market-regime">${esc(label)}</div>`:''}<p class="v1-ai-summary">${esc(comment||'Analisi in aggiornamento…')}${strategy?` <span class="v1-strategy-context">${esc(strategy)}</span>`:''}</p>`;
  }
  ...
  async function fetchAI(){
    ...
    try{
      if(strategy){
        const post=await fetch('/api/ai?ts='+Date.now(),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({strategyConsensus:strategy}),cache:'no-store'});
        if(post.ok){const payload=await post.json();if(payload?.ai)return payload.ai;}
      }
  ...
  async function renderAI(){
    ...
    if(!box)return;
    box.innerHTML='<div class="v1-ai-loading">Lettura di mercato, news, cross-asset e Strategy Lab in corso…</div>';
    const ai=await fetchAI();
    if(ai){box.innerHTML=renderMarketText(ai);return;}
    const strategy=strategyNarrative();
    box.innerHTML=`<div class="v1-ai-loading">Analisi AI in aggiornamento.${strategy?`<br><br><strong>${esc(strategy)}</strong>`:''}</div>`;
  }
  ...
  async function manualNewsSearch(btn){
    ...
    try{await window.ML.data.forceNewsScan();await renderAI();}
  ...
  function technicalPulse(x){
    ...
    const macd=Number(x.macd??x.MACD??x.macdHist);if(Number.isFinite(macd))parts.push(`MACD ${macd>0?'+':macd<0?'−':'0'}`);
    const mom=Number(x.momentum??x.mom??x.MOM??x.momentumPct);if(Number.isFinite(mom))parts.push(`MOM ${mom>0?'↑':mom<0?'↓':'→'}`);
  ...
  function renderIndices(market){
    const all=(market?.indices||[]).filter(x=>x.ok);if(!all.length)return '<div class="v1-empty">Dati non disponibili.</div>';
    const sections=new Map();all.forEach(x=>{const [label,key]=indexGroup(x.name);if(!sections.has(key))sections.set(key,{label,items:[]});sections.get(key).items.push(x);});
    const order=['usa','europe','asia','risk','commodities','fx','rates','other'];let html='';
    order.forEach(key=>{const s=sections.get(key);if(!s)return;s.items.sort((a,b)=>String(a.name).localeCompare(String(b.name)));html+=`<div class="v1-index-group"><div class="v1-index-group-title">${s.label}<span>${s.items.length}</span></div><div class="v1-index-grid">${s.items.map(x=>{const ch=Number(x.changePct),value=x.price??x.value??x.close;return`<div class="v1-index"><div class="v1-index-main"><strong>${esc(x.name)}</strong><span>${Number.isFinite(ch)?(ch>=0?'+':'')+ch.toFixed(2)+'%':'—'}</span><small>${Number.isFinite(Number(value))?Number(value).toLocaleString('it-IT',{maximumFractionDigits:2}):'—'}</small></div><div class="v1-index-tech">${esc(technicalPulse(x))}</div></div>`}).join('')}</div></div>`});
    return html;
  }
  ...
  window.addEventListener('miracolo:strategyupdate',()=>{if(document.getElementById('v1-market-content'))renderAI().catch(e=>console.warn('Strategy → Market Sentiment:',e.message));});
})();
