...
(() => {
  ...
  function marketAnalysis(market,items){
    ...
    if(vix&&num(vix.changePct)>2)risks.push('la volatilità sta aumentando');if(gold&&num(gold.changePct)>1)risks.push('l’oro mostra una domanda difensiva');if(us10&&num(us10.changePct)>1)risks.push('i rendimenti USA stanno salendo');
  ...
  async function aiRecap(data,ma){const fb=fallback(ma),key=typeof CFG!=='undefined'?CFG.geminiKey:'';if(!key)return fb;const items=(data.scan?.items||[]).slice(0,150),indices=(data.market?.indices||[]).filter(x=>x.ok).map(x=>({name:x.name,changePct:Number(x.changePct),rsi:x.rsi})),crypto=(data.market?.crypto||[]).filter(x=>x.ok).map(x=>({symbol:x.symbol,change24h:x.change24h})),headlines=items.map(x=>`[${x.source||'Fonte'}][${group(x)}] ${x.title||''}`).join('\n');const prompt=`Sei l'analista AI di Miracolo Lab. Devi produrre il recap operativo del mercato sulla base delle informazioni ricevute. Considera congiuntamente indici, news, finance, crypto, macroeconomia, obbligazioni/tassi, banche centrali, commodities, forex, volatilità, geopolitica, social/sentiment e società/earnings. Dai priorità a eventi market-moving e convergenze/divergenze. Non inventare dati. Scrivi in italiano semplice ma professionale.\n\nINDICI:\n${JSON.stringify(indices)}\nCRYPTO:\n${JSON.stringify(crypto)}\nFONTI/HEADLINES:\n${headlines}\n\nRestituisci SOLO JSON valido: {"headline":"una frase di sintesi","summary":"80-140 parole sul quadro e sui principali driver","bullish":"massimo 2 frasi sui fattori favorevoli","bearish":"massimo 2 frasi sui rischi/fattori contrari","watch":"massimo 2 frasi su cosa monitorare"}`;try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(key)}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:.2,maxOutputTokens:600}})});if(!r.ok)throw Error('Gemini HTTP '+r.status);const d=await r.json(),text=d.candidates?.[0]?.content?.parts?.[0]?.text||'',m=text.match(/\{[\s\S]*\}/);if(!m)throw Error('AI JSON non valido');const o=JSON.parse(m[0]);return{headline:o.headline||fb.headline,summary:o.summary||fb.summary,bullish:o.bullish||fb.bullish,bearish:o.bearish||fb.bearish,watch:o.watch||fb.watch,source:'AI Gemini'};}catch(e){console.warn('V1 Market Sentiment AI:',e.message);return fb;}}
  let lastKey='',requestId=0;
  function renderMarketText(r,ma){return `<div class="v1-ai-headline">${esc(r.headline)}</div><p class="v1-ai-summary">${esc(r.summary)}</p>${r.bullish?`<div class="v1-ai-block"><b>Fattori favorevoli</b><span>${esc(r.bullish)}</span></div>`:''}${r.bearish?`<div class="v1-ai-block"><b>Rischi / fattori contrari</b><span>${esc(r.bearish)}</span></div>`:''}${r.watch?`<div class="v1-ai-block"><b>Da monitorare</b><span>${esc(r.watch)}</span></div>`:''}<div class="v1-ai-foot">${esc(r.source)} · ${ma.newsCount} elementi informativi nel dataset</div>`;}
  async function renderAI(data,ma){const box=document.getElementById('v1-market-content');if(!box||!ma)return;const key=(data.scan?.timestamp||data.market?.timestamp||'')+'|'+(data.scan?.items?.length||0)+'|'+ma.score;if(key===lastKey)return;lastKey=key;const id=++requestId;const fb=fallback(ma);box.innerHTML=renderMarketText(fb,ma)+'<div class="v1-ai-loading" style="margin-top:8px">Aggiornamento AI in corso…</div>';const r=await aiRecap(data,ma);if(id!==requestId)return;box.innerHTML=renderMarketText(r,ma);}
