...
(() => {
  ...
  async function renderAI(data,ma){const box=document.getElementById('v1-market-content');if(!box||!ma)return;const key=(data.scan?.timestamp||data.market?.timestamp||'')+'|'+(data.scan?.items?.length||0)+'|'+ma.score;if(key===lastKey)return;lastKey=key;const id=++requestId;box.innerHTML='<div class="v1-ai-loading">Analisi AI del flusso ricevuto in corso…</div>';const r=await aiRecap(data,ma);if(id!==requestId)return;box.innerHTML=`<div class="v1-ai-headline">${esc(r.headline)}</div><p class="v1-ai-summary">${esc(r.summary)}</p>${r.bullish?`<div class="v1-ai-block"><b>Fattori favorevoli</b><span>${esc(r.bullish)}</span></div>`:''}${r.bearish?`<div class="v1-ai-block"><b>Rischi / fattori contrari</b><span>${esc(r.bearish)}</span></div>`:''}${r.watch?`<div class="v1-ai-block"><b>Da monitorare</b><span>${esc(r.watch)}</span></div>`:''}<div class="v1-ai-foot">${esc(r.source)} · ${ma.newsCount} elementi informativi nel dataset</div>`;}
