/* Miracolo Lab V21.2.1 — source intelligence UI. */
(() => {
  'use strict';
  ...
  function renderSources(){const p=ensure();if(!p)return;const sources=typeof S!=='undefined'?S.sources||[]:[];const ok=sources.filter(x=>x.status==='ok').length,err=sources.filter(x=>x.status==='error').length,off=sources.filter(x=>x.status==='disabled').length;const groups={};sources.forEach(x=>(groups[x.type||x.cat||'other']??=[]).push(x));p.innerHTML=`<summary>📚 FONTI RICEVUTE <span class="ml-sp-count">${ok} OK · ${err} ERR · ${off} OFF · ${sources.length} totali</span></summary><div id="mlSourceCategories"></div><div class="ml-sp-body">${Object.entries(groups).map(([g,arr])=>`<div class="ml-sp-group"><div class="ml-sp-group-title">${cat(g)} ${esc(g)}</div>${arr.map(s=>{const cls=s.status==='ok'?'ml-sp-ok':s.status==='disabled'?'ml-sp-off':'ml-sp-err';const label=s.status==='ok'?`OK · ${Number(s.count||0)}`:s.status==='disabled'?'OFF':'ERR';const extra=[s.latencyMs?`${s.latencyMs}ms`:'',s.error?s.error:''].filter(Boolean).join(' · ');return `<div class="ml-sp-row"><span class="ml-sp-name">${s.url?`<a href="${esc(s.url)}" target="_blank" rel="noopener">${cat(g)} ${esc(s.name)}</a>`:`${cat(g)} ${esc(s.name)}`}<span class="ml-sp-meta">${extra?` · ${esc(extra)}`:''}</span></span><span class="${cls}">${label}</span></div>`}).join('')}</div>`).join('')}</div>`;window.dispatchEvent(new CustomEvent('miracolo:source-menu-ready'))}
  function renderMoreFeed(){
    if(typeof S==='undefined'||typeof signalCard!=='function')return;
    const filter=S.filter||'all';let list=S.signals.slice();
    ...
    const groups=[['🔥','Segnali Forti',list.filter(x=>Number(x.score||0)>=5)],['🌍','Macro & Economia',list.filter(x=>x.cat==='macro'&&Number(x.score||0)<5)],['📰','Mercati & Finance',list.filter(x=>x.cat==='news'&&x.kind==='news'&&Number(x.score||0)<5)],['₿','Crypto News',list.filter(x=>x.cat==='crypto'&&Number(x.score||0)<5)],['💬','Social & Reddit',list.filter(x=>x.kind==='social'&&Number(x.score||0)<5)]].filter(g=>g[2].length);
    const r=document.getElementById('results');if(!r)return;let html='';
    groups.forEach((g,gi)=>{const id=`mlfg${gi}`;html+=`<details class="ml-feed-group" open><summary>${g[0]} ${g[1]} <span style="float:right;color:#64748b">${g[2].length}</span></summary><div class="ml-feed-items">${g[2].slice(0,50).map((x,i)=>signalCard(x,i)).join('')}${g[2].length>50?`<button class="ml-more" data-group="${id}">Mostra altri ${Math.min(100,g[2].length-50)}</button>`:''}</div></details>`});
    r.innerHTML=html;
    r.querySelectorAll('.ml-more').forEach(btn=>btn.addEventListener('click',()=>{const parent=btn.closest('.ml-feed-group'), title=parent?.querySelector('summary')?.textContent||'';const source=groups.find(g=>title.includes(g[1]));if(!source)return;const items=parent.querySelector('.ml-feed-items');items.innerHTML=source[2].slice(0,100).map((x,i)=>signalCard(x,i)).join('');if(source[2].length>100){const b=document.createElement('button');b.className='ml-more';b.textContent=`Mostra altri ${source[2].length-100}`;b.onclick=()=>{items.innerHTML=source[2].map((x,i)=>signalCard(x,i)).join('')};items.appendChild(b)}}));
  }
  function hook(){style();renderSources();renderMoreFeed();}
  ...
  setInterval(()=>{if(typeof S!=='undefined'&&S.sources?.length)renderSources()},5000);
})();
