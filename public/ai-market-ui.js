...
(() => {
  ...
  const applyState = (state) => { hideLegacy(); if (state?.ai) render(state.ai); };
  const load = async () => { try { const response = await fetch('/api/live?ai=1', { cache: 'no-store' }); if (!response.ok) return; applyState(await response.json()); } catch {} };
  const boot = () => { hideLegacy(); load(); if (window.ML?.on) window.ML.on('state', applyState); };
